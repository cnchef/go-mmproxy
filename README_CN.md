...
以下是几个 **go-mmproxy** 的常用启动用法示例，假设你已经构建好了可执行文件（如 `go-mmproxy`）：

---

## ✅ 示例 1：将本地 `8000` 端口的服务对外暴露为 `20000`（IPv4）
...
| ------------------- | ------------------------------------------ |
| `-p tcp`            | 代理 TCP 协议                                  |
| `-l 0.0.0.0:20000`  | 监听所有网卡的 20000 端口                           |
| `-4 127.0.0.1:8000` | 将 IPv4 流量转发到本机的 8000 端口                    |

---

## ✅ 示例 2：UDP 协议代理本地 8001 端口到外部 20001
...
UDP 模式会为每个源地址打开一个会话，`--close-after` 表示在 30 秒无数据后清理这个会话。
...
* 会创建 4 个独立监听 socket，提升高并发连接下的负载能力（依赖 Linux 3.9+ 支持的 `SO_REUSEPORT`）。

---

## ✅ 示例 4：限制允许连接来源（仅来自配置文件中 IP 段）
